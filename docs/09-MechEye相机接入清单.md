...
2026-09-14 使用 `scripts/validate_mecheye_capture.py` 独立复核：普通点云
共 `1310720` 点，有限点 `747804`（`57.0529%`），XYZ 范围为
`[-0.521240,0.294094]`、`[-0.058375,0.278654]`、
`[0.201726,1.719370] m`，字段为三个偏移 `0/4/8` 的 FLOAT32，
`point_step=12`。纹理点云有限点 `747517`（`57.0310%`），但所有有限点的
B/G/R 仍完全相等；它也不包含真实颜色。原始统计保存在
