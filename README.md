...
2026-09-24 的视觉自主抓取验收、相机依赖恢复和完整命令见
...
| 位置 | 用途 |
|---|---|
| `scripts/` | 采集、规划、真机门禁与执行脚本 |
| `config/` | 标定、工具、附件与网络配置 |
| `ros2_ws/src/` | 数字孪生与 ROS 包完整源码 |
| `third_party/` | 外部 Mech-Eye ROS 包的固定版本补丁与恢复启动文件 |
| `docs/`、`HANDOVER.md` | 当前操作入口、限制与交接说明 |
| `outputs/` | 已版本化的原始数据、计划和实验验证证据 |
...
- 固定点课程演示可显式使用 `--demo`；该模式忽略 `OBJ`，不代表抓取成功，完整重启后指令见 `docs/00-当前可用操作.md`。
...
| [`docs/16-历史ROS桥抓放方法.md`](docs/16-历史ROS桥抓放方法.md) | 原样保留的旧 ROS 抓放命令，仅供历史对照 |
