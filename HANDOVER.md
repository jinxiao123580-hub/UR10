...
- 2026-09-17 完成 eye-in-hand 棋盘格手眼标定：`10×7` 方格、`9×6` 内角点、
  格长 `6 mm`。20 个有效姿态中前 15 个拟合、后 5 个独立验证。Park 解的留出
  闭环 RMS 为 `2.642 mm / 1.051°`，最大 `3.855 mm / 1.837°`；Park 与 Horaud
...
- 棋盘格 PnP 与组织点云逐角点几何验收 54/54 有效，XYZ 误差 RMS `0.692 mm`、
  ...
  完成真实物体抓取点的物理触达验证。
- 固定棋盘格旁的标称 50 mm 立方体已得到视觉候选中心：base 系约
  `[0.63025,-0.09923,0.01388] m`。该值由可见竖直侧面加已知边长推断，尚不是完整
  三维包围盒实测，状态为 `candidate_not_motion_authorized`；下一步只允许先验证安全
  悬停点，不能直接下降抓取。
- 2026-09-17 完成一次“视觉候选 + 人工修正”的抓取、提升 20 mm、原位放回闭环。
  初始视觉候选与用户手动确认的实际抓取中心 XY 相差 `35.84 mm`，因此**全自动视觉
  定位仍未通过**。修正后夹爪 `POS=102 / OBJ=2 / FLT=0`，用户确认物块离地；下降
  放回后张开至 `POS=3`。全程安全监控无异常。
...
│   ├── 12-力传感器重力补偿.md    已验证数据通道；当前补偿参数禁用
